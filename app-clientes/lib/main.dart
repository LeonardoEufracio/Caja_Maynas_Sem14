...
class AppColors {
  static const green = Color(0xFFE30613);
  static const deepGreen = Color(0xFFB8000A);
  static const lime = Color(0xFFFFFFFF);
...
const demoDni = '72079156';
const demoEmail = 'alumno1@example.com';
...
final demoProfile = UserProfile(
  ...
  address: 'Jr. Prospero 791, Iquitos',
...
final demoProducts = BankProducts(
  ...
  movements: const [
    ...
    Movement('Pago tarjeta Caja Maynas', 'Ayer, 19:44', 180.90, false),
...
class UserProfile {
  ...
  factory UserProfile.fromCoreSummary(Map<String, dynamic> json) {
    ...
    final fullName = [
      ...
      _text(cliente['apellidos'], 'Caja Maynas'),
    ].where((part) => part.trim().isNotEmpty).join(' ');
    return UserProfile(
      name: fullName,
      email: _text(cliente['email'], '$dni@cliente.cajamaynas.pe'),
...
class BankProducts {
  ...
  factory BankProducts.fromCoreSummary(Map<String, dynamic> json) {
    ...
    return BankProducts(
      savings: SavingsAccount(
        name: _text(cuenta['tipo_cuenta'], 'Ahorro Corriente'),
...
class CoreApiClient {
  ...
  Future<CreditApplicationResult> submitApplication(
    ...
    try {
      return _postApplication(primaryCoreBaseUrl, body);
    } catch (_) {
      return _postApplication(fallbackCoreBaseUrl, body);
    }
  }

  Future<List<Map<String, dynamic>>> loadCases() async {
    try {
      return _getCases(primaryCoreBaseUrl);
    } catch (_) {
      return _getCases(fallbackCoreBaseUrl);
    }
  }
  ...
  Future<String> loginCliente(String document, String password) async {
    ...
    try {
      return _postLogin(primaryCoreBaseUrl, body);
    } catch (_) {
      return _postLogin(fallbackCoreBaseUrl, body);
    }
  }
  ...
  Future<Map<String, dynamic>> loadCustomerSummary(String document) async {
    try {
      ...
      return _getSummary(fallbackCoreBaseUrl, document);
    }
  }
      ...
      body: Container(
        decoration: const BoxDecoration(
          gradient: LinearGradient(
            colors: [Color(0xFFE8F5E9), Color(0xFFF7FAF2)],
      ...
      child: Column(
        ...
        children: [
          ...
          FilledButton.icon(
            ...
            label: const Text('Solicitar microcredito Ahorita'),
          ),
          const SizedBox(height: 18),
...
class _CreditApplicationPageState extends State<CreditApplicationPage> {
  ...
  Future<void> loadPdfCases() async {
    try {
  ...
  void applyCase(int caseNumber) {
    ...
    setState(() {
      ...
      termMonths = data.termMonths == 0 ? 12 : data.termMonths;
      tea = data.tea == 0 ? 0.4392 : data.tea;
...
class MaynasLogo extends StatelessWidget {
  const MaynasLogo({super.key, this.compact = false});

  final bool compact;

  @override
  Widget build(BuildContext context) {
    final badgeWidth = compact ? 210.0 : 270.0;
    final badgeHeight = compact ? 76.0 : 96.0;
    return SizedBox(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.only(
              topLeft: Radius.circular(compact ? 20 : 26),
              bottomRight: Radius.circular(compact ? 20 : 26),
              topRight: Radius.circular(6),
              bottomLeft: Radius.circular(6),
            ),
            boxShadow: [
              BoxShadow(
                color: AppColors.deepGreen.withOpacity(0.22),
                blurRadius: 18,
                offset: const Offset(0, 10),
              ),
            ],
          ...
          child: Row(
            ...
            children: [
              SizedBox(
                ...
                child: const CustomPaint(painter: MaynasMarkPainter()),
              ),
              const SizedBox(width: 12),
              Text(
                'CAJA\nMAYNAS',
...
class MaynasMarkPainter extends CustomPainter {
  const MaynasMarkPainter();

  @override
  void paint(Canvas canvas, Size size) {
    final whitePaint = Paint()..color = Colors.white;
    final cutPaint = Paint()..color = AppColors.green;
...
class AppCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      padding: const EdgeInsets.all(18),
      decoration: boxDecoration(accent: accent),
      child: child,
    );
  }
}
